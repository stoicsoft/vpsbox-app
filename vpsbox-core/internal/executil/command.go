...
// Resolve returns the absolute path to a binary, searching the enriched PATH, or
// "" when it is not installed. Callers that build their own exec.Cmd — long-lived
// detached processes, which Run cannot manage — need this to find the binary the
// same way Run would.
func Resolve(bin string) string {
	return resolveExecutable(bin)
}

// Env returns the process environment with the enriched PATH applied, for callers
// building their own exec.Cmd.
func Env() []string {
	return withEnrichedPath()
}
