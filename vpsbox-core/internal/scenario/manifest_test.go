...
func TestCommittedHTTPReverseProxyScenariosAreValid(t *testing.T) {
	paths, err := filepath.Glob(filepath.Join("..", "..", "scenarios", "http-reverse-proxy-compat", "*.yaml"))
	if err != nil {
		t.Fatal(err)
	}
	if len(paths) != 1 {
		t.Fatalf("expected one HTTP reverse-proxy scenario, got %d", len(paths))
	}
	for _, path := range paths {
		...
		if len(manifest.Spec.Instances) != 3 {
			t.Fatalf("%s: expected three proxy fixtures, got %d", path, len(manifest.Spec.Instances))
		}
	}
}
