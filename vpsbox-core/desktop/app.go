...
func (a *DesktopApp) newJob(kind, target string) *Job {
	a.mu.Lock()
	defer a.mu.Unlock()
...
func (a *DesktopApp) runJob(id string, fn func(job *Job) error) {
	job := a.getJob(id)
	if job == nil {
		return
	}
...
func (a *DesktopApp) finishJob(id, state, message string) {
	a.mu.Lock()
	defer a.mu.Unlock()
...
func (a *DesktopApp) updateJobMessage(id, message string) {
	a.mu.Lock()
	defer a.mu.Unlock()
	if job, ok := a.jobs[id]; ok {
		job.Message = message
...
func (a *DesktopApp) getJob(id string) *Job {
	a.mu.Lock()
	defer a.mu.Unlock()
	return a.jobs[id]
}

func (a *DesktopApp) setJob(job *Job) {
	a.mu.Lock()
	defer a.mu.Unlock()
	a.jobs[job.ID] = job
}

func (a *DesktopApp) listJobs() []Job {
	a.mu.Lock()
	defer a.mu.Unlock()
