...
function App() {
  ...
  return (
    <div className="app">
      ...
      {viewingKeys ? (
        <SSHKeyModal keys={viewingKeys} onClose={() => setViewingKeys(null)} />
      ) : null}
          ...
          <div className="connect-actions">
            ...
            {selectedInstance.hasPrivateKey ? (
              <>
                ...
                <Button onClick={() => void RevealKeyFolder(selectedInstance.name)}>
                  Reveal folder
                </Button>
              </>
            ) : null}
  ...
  return (
    <div className="overlay" onClick={props.onClose} role="presentation">
      ...
      >
        <header className="dialog-head">
          <div>
            <span className="eyebrow">SSH Keys</span>
            <h2>View key</h2>
          </div>
          <button
            type="button"
        ...
        <div className="key-tabs">
          <button
            type="button"
            className={`key-tab ${tab === 'public' ? 'active' : ''}`}
          ...
          <button
            type="button"
            className={`key-tab ${tab === 'private' ? 'active' : ''}`}
            onClick={() => setTab('private')}
          >
            Private key
          </button>
        </div>
        <pre className="key-content"><code>{content}</code></pre>
        <div className="dialog-actions">
