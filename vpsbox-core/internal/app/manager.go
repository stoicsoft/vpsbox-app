...
func NewManager(ctx context.Context) (*Manager, error) {
	...
	buckets := objstore.NewManager(paths, store)
...
func (m *Manager) Destroy(ctx context.Context, name string, force bool) error {
	...
	if err := m.backend.Delete(ctx, instance.Name); err != nil && !backend.IsAbsentInstance(err) {
		return err
	}
		...
		if err == nil {
			...
			for i := range instances {
				if latest, ok := byName[instances[i].Name]; ok {
					...
					instances[i].Status = string(backend.StatusUnknown)
				}
			}
