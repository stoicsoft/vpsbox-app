...
type UpdateDownload = {
  state: 'idle' | 'downloading' | 'preparing' | 'ready' | 'error';
  version?: string;
  percent: number;
  downloaded: number;
  total: number;
...
// One app published by the sandbox: something we installed, or any other
// container publishing a port (an app deployed through Coolify, say).
type LiveApp = {
  templateId?: string;
  name: string;
  icon?: string;
...
// Mirrors desktopbackend.TransferPlan: what a push/import migration will do.
type TransferPlan = {
  source: string;
  dest: string;
  packages?: string[];
  services?: string[];
  users?: string[];
  paths?: { path: string; sizeMB: number }[];
  volumes?: { name: string; sizeMB: number }[];
  compose?: { name: string; running: boolean }[];
  notes?: string[];
  estimatedMB: number;
};

type VPSTargetValues = {
  user: string;
  host: string;
  port: number;
  keyPath: string;
...
// Mirrors desktopbackend.ImportPreview: what an import will do, learned by
// only reading the server.
type ImportPreview = {
  server: string;
  os: string;
  sandboxName: string;
...
const initialState: AppState = {
  ...
  instances: [],
  jobs: [],
  updateDownload: { state: 'idle', percent: 0, downloaded: 0, total: 0 },
};
...
// stageStateAt is the shape-agnostic core: given where the job currently is,
// classify one stage. Each flow supplies its own message → index mapping.
function stageStateAt(stageIndex: number, current: number, jobState: string | undefined): StageState {
  if (jobState === 'error') {
...
const PUSH_STAGES = [
  { id: 'inspect', title: 'Inspect both machines', description: 'Read what each machine has and work out the difference.' },
  { id: 'prepare', title: 'Install packages', description: 'Copy repo configuration and install what is missing.' },
  { id: 'files', title: 'Copy files', description: 'Stream your files onto the server.' },
  { id: 'data', title: 'Move app data', description: 'Copy docker volume data.' },
  { id: 'services', title: 'Start services and apps', description: 'Enable services and start compose projects.' },
] as const;

// pushStageIndex maps the backend's Status messages onto PUSH_STAGES. The
// message strings come from Manager.applyMigration — change them together.
function pushStageIndex(message: string, state: string): number {
  const m = message.toLowerCase();
  if (state === 'done' || m.includes('push complete') || m.startsWith('pushed ')) return PUSH_STAGES.length;
  if (m.includes('enabling') || m.includes('compose')) return 4;
  if (m.includes('stopping containers') || m.includes('docker volume')) return 3;
  if (m.includes('copying files')) return 2;
  if (m.includes('repository') || m.startsWith('preparing')) return 1;
  return 0;
}

const IMPORT_STAGES = [
  { id: 'inspect', title: 'Inspect the server', description: 'Read what runs there — nothing on it changes.' },
  { id: 'create', title: 'Create the sandbox', description: 'Boot a local VM sized to hold it.' },
  { id: 'prepare', title: 'Install packages', description: 'Install what the server had.' },
  { id: 'files', title: 'Copy files', description: 'Stream the server’s files in.' },
  { id: 'data', title: 'Move app data', description: 'Copy docker volume data.' },
  { id: 'services', title: 'Start services and apps', description: 'Enable services and start compose projects.' },
] as const;

function importStageIndex(message: string, state: string): number {
  const m = message.toLowerCase();
  if (state === 'done' || m.includes('import complete') || m.includes('local copy of')) return IMPORT_STAGES.length;
  if (m.includes('enabling') || m.includes('compose')) return 5;
  if (m.includes('stopping containers') || m.includes('docker volume')) return 4;
  if (m.includes('copying files')) return 3;
  if (m.includes('repository') || m.startsWith('preparing ')) return 2;
  // Everything Up() reports while the VM boots, plus the post-boot inspection.
  if (
    m.includes('creating sandbox') ||
    m.includes('new sandbox') ||
    m.includes('backend') ||
    m.includes('ssh key') ||
    m.includes('bootstrap') ||
    m.includes('launching') ||
    m.includes('waiting') ||
    m.includes('finalizing') ||
    m.includes('registry') ||
    m.includes('existing sandbox')
  ) {
    return 1;
  }
  return 0;
...
function stageStateFor(stageIndex: number, job: Job | undefined): StageState {
  if (!job) return 'idle';
  return stageStateAt(stageIndex, createStageIndex(job.message, job.state), job.state);
}
...
function jobLabel(kind: string): string {
  switch (kind) {
    ...
    case 'push':
      return 'Pushing to VPS';
    case 'import':
      return 'Importing VPS';
    case 'domains':
      return 'Updating /etc/hosts';
    case 'app-update':
      return 'Downloading VPSBox update';
    case 'bootstrap':
...
// updateHeadline is the one line that describes where the update stands. The
// banner and the System card show the same words so the two never disagree.
function updateHeadline(props: UpdateActionProps): string {
  const { update, download, restarting } = props;
  if (restarting) return 'Restarting to finish the update…';
  switch (download.state) {
    case 'downloading':
      return download.total > 0
        ? `Downloading ${formatBytes(download.downloaded)} of ${formatBytes(download.total)}`
        : `Downloading VPSBox ${download.version ?? update.latest}`;
    case 'preparing':
      return 'Preparing the update…';
    case 'ready':
      return `VPSBox ${download.version ?? update.latest} is ready to install.`;
    case 'error':
      return download.error || 'The update could not be downloaded.';
    default:
      return update.canInstall
        ? `VPSBox can install ${update.latest} for you.`
        : `VPSBox ${update.latest} is available to download.`;
  }
}

// UpdateActions renders the buttons for whatever stage the update is at. It's
// shared by the banner and the System screen: one place decides what "install"
// means right now, so both surfaces stay in step.
function UpdateActions(props: UpdateActionProps) {
  const { update, download, busy, restarting } = props;
  ...
  if (download.state === 'downloading' || download.state === 'preparing') {
    return (
      <Button icon="close" onClick={props.onCancel} busy={Boolean(busy['update-cancel'])}>
        Cancel
      </Button>
    );
  }
  ...
  if (download.state === 'ready') {
    return (
      <Button
        variant="primary"
        icon="refresh"
        busy={Boolean(busy['update-apply'])}
  ...
  // Failed downloads keep the retry button but also surface the manual route,
  // because the cause is often something retrying won't fix.
  if (download.state === 'error') {
    return (
      <>
        <Button icon="refresh" busy={Boolean(busy['update-download'])} onClick={props.onDownload}>
          Try again
        </Button>
        <Button icon="download" onClick={() => void OpenExternal(update.url)}>
          Download in browser
        </Button>
      </>
    );
  ...
  if (update.canInstall) {
    return (
      <Button
        variant="primary"
        icon="download"
        busy={Boolean(busy['update-download'])}
  ...
  return (
    <Button variant="primary" icon="download" onClick={() => void OpenExternal(update.url)}>
      Download v{update.latest}
    </Button>
  );
}

// UpdateProgress is the download bar. It only appears while bytes are moving —
// a finished or failed download says so in words instead.
function UpdateProgress({ download }: { download: UpdateDownload }) {
  if (download.state !== 'downloading' && download.state !== 'preparing') return null;
  const indeterminate = download.state === 'preparing' || download.total <= 0;
  return (
    <div className="meter update-progress">
      <div className="meter-track">
        <div
          className={`meter-fill ${indeterminate ? 'meter-fill-pulse' : ''}`}
          style={{ width: indeterminate ? '100%' : `${download.percent}%` }}
        />
      </div>
      <span>{indeterminate ? 'Preparing' : `${download.percent}%`}</span>
    </div>
  );
}
...
function App() {
  ...
  const [pushing, setPushing] = useState<Sandbox | null>(null);
  ...
  // Applying an update quits the app; the flag keeps the UI from offering the
  // button twice while the window is on its way out.
  ...
  const updateActions = useMemo(
    () => ({
      onDownload: () => {
        ...
        void runAction('update-download', () => StartDownloadUpdate());
      },
      onCancel: () => void runAction('update-cancel', () => CancelUpdateDownload()),
      // Applying goes straight to the binding rather than through runAction:
      // on success the app quits a moment later and there is no state left to
      // refresh, and on failure the button has to come back.
      onApply: () => {
        setRestarting(true);
        setError('');
  ...
  return (
    <div className={`app ${IS_MAC ? 'app-mac' : ''}`}>
            ...
            {section === 'servers' ? (
              <>
                <ToolbarButton
                  ...
                  title="Clone a real VPS into a local sandbox"
                >
                  Import from VPS
                </ToolbarButton>
                <ToolbarButton
        ...
        <main className="detail">
          ...
          {state.update?.available && !updateDismissed ? (
            <div className="banner banner-update" role="status">
              <div className="banner-body">
                ...
                <UpdateProgress download={state.updateDownload} />
              </div>
              <span className="banner-ops">
                <UpdateActions
                  update={state.update}
                  download={state.updateDownload}
                  busy={actionsBusy}
              ...
              {/* Dismissing hides the banner, not the update: the System
                  screen keeps the same controls for later. */}
          ...
          {bootstrapped && !showFirstRun && section === 'servers' ? (
            <ServersScreen
              ...
              onPush={(instance) => setPushing(instance)}
      ...
      {pushing ? (
        <PushSheet instance={pushing} jobs={state.jobs} onClose={() => setPushing(null)} />
      ) : null}
    ...
    <>
      <header className="detail-head">
        ...
        <div className="detail-ops">
          ...
          <Button
            icon="external"
            disabled={!isRunning || !selectedInstance.hasPrivateKey}
            onClick={() => props.onPush(selectedInstance)}
            title={
              !selectedInstance.hasPrivateKey
                ? 'Generate an SSH key first'
                : !isRunning
                  ? 'Start the server first'
                  : 'Copy this sandbox onto a real VPS over SSH'
            }
          >
            Push to VPS
          </Button>
...
function ServerDeployTab({ instance, jobs }: { instance: Sandbox; jobs: Job[] }) {
  ...
  // What the sandbox actually publishes right now, read back over SSH.
  const [apps, setApps] = useState<LiveApp[] | null>(null);
  ...
  const loadApps = useCallback(async () => {
    ...
    setAppsLoading(true);
    try {
      const result = await ListApps(instance.name);
      setApps((result ?? []) as unknown as LiveApp[]);
      setAppsError('');
    } catch (err) {
      setAppsError(toMessage(err));
    } finally {
  ...
  // Clear the spinner when the deploy job finishes (was running, now gone), and
  // re-read what the sandbox publishes — that install is why the list changed.
  ...
  const appTemplates = (templates ?? []).filter((t) => t.category === 'app');
    ...
    <div className="detail-scroll">
      <div className="pane">
        ...
        {!isRunning ? (
          ...
          <>
            <Inspector title="Deploy platforms" flush>
            ...
            <Inspector title="Apps & tools" flush>
              <div className="tpl-list">{appTemplates.map(renderCard)}</div>
            </Inspector>
            ...
            <Inspector
              ...
              flush
              action={
                <Button icon="refresh" busy={appsLoading} onClick={() => void loadApps()}>
                  Refresh
                </Button>
              }
            >
              <LiveAppTable apps={apps} error={appsError} />
            </Inspector>
          </>
        )}
...
// What the sandbox publishes right now — one row per port, each with the URL
// the app answers on and a button that opens it in the host browser.
function LiveAppTable({ apps, error }: { apps: LiveApp[] | null; error: string }) {
  if (error) {
    return (
      <div className="pad-blank">
  ...
  if (apps === null) {
    return (
      <div className="pad-blank">
        <p className="muted">Checking what's running…</p>
      </div>
    );
  }
  if (apps.length === 0) {
    return (
      <div className="pad-blank">
        <p className="muted">
          Nothing published yet. Deploy something above and it will show up here with a link.
        </p>
      </div>
    );
  }
  return (
    <div className="apps">
      <div className="app-row app-head">
        <span>App</span>
        <span>Port</span>
        <span>Address</span>
        <span>Status</span>
      ...
      {apps.map((app) => (
        <div className="app-row" key={`${app.name}-${app.port}`}>
          ...
          <StatusChip status={app.running ? 'running' : 'stopped'} />
          <Button
            icon="external"
            disabled={!app.running || !app.url}
            onClick={() => void OpenExternal(app.url)}
            title={app.running ? `Open ${app.url}` : 'Nothing is listening on this port'}
          >
            View app
          </Button>
        </div>
...
function SystemScreen(props: {
  ...
  actionsBusy: Record<string, boolean>;
  restarting: boolean;
  updateActions: { onDownload: () => void; onCancel: () => void; onApply: () => void };
  onInstall: () => void;
  onFixDomains: () => void;
  onCheckUpdate: () => void;
}) {
            ...
            {props.update?.error ? (
              ...
              <>
                <p>
                  {updateHeadline({
                    update: props.update,
                    download: props.updateDownload,
                    busy: props.actionsBusy,
                    restarting: props.restarting,
                    ...props.updateActions,
                  })}
                </p>
                <UpdateProgress download={props.updateDownload} />
                {/* Say why the in-app install is unavailable rather than
                    silently degrading to a browser download. */}
                {props.update.blocker && props.updateDownload.state === 'idle' ? (
                  <p className="hint">{props.update.blocker}</p>
                ) : null}
              </>
            ) : (
              <p>
...
function FeedRow({ job }: { job: Job }) {
  // Running jobs open by default: while an install streams, the log is the
  // interesting part. Once opened or closed by hand, the choice sticks.
  const [open, setOpen] = useState(job.state === 'running');
  const log = job.log ?? [];
  const logText = log.join('\n');
  const logRef = useRef<HTMLPreElement | null>(null);

  // Follow the tail as new lines stream in, the way a terminal would.
  useEffect(() => {
  ...
  return (
    <div className={`feed-item ${open ? 'feed-item-open' : ''}`}>
      <div className="feed-row">
        ...
        {log.length > 0 ? (
          <button
            ...
            aria-expanded={open}
            title={open ? 'Hide the output of this job' : 'Show the output of this job'}
          >
            <Icon name="lines" size={12} />
            <span>{open ? 'Hide log' : `Log (${log.length})`}</span>
          </button>
        ) : (
          <span className="feed-toggle-blank" />
        )}
      </div>
      {open && log.length > 0 ? (
        <pre className="feed-log" ref={logRef}>
          {logText}
        </pre>
      ) : null}
    </div>
...
const DEFAULT_VPS_TARGET: VPSTargetValues = { user: 'root', host: '', port: 22, keyPath: '' };

// The last-used target is remembered per flow: pushing usually goes to the
// same server every time, and retyping a host and key path is pure friction.
function loadStoredTarget(key: string): VPSTargetValues {
  try {
    const raw = window.localStorage.getItem(key);
    if (raw) return { ...DEFAULT_VPS_TARGET, ...(JSON.parse(raw) as Partial<VPSTargetValues>) };
  } catch {
...
function saveStoredTarget(key: string, values: VPSTargetValues) {
  try {
...
// The SSH coordinates of a real VPS — shared by the push and import sheets.
function VPSTargetFields(props: {
  values: VPSTargetValues;
  onChange: (values: VPSTargetValues) => void;
}) {
  const { values, onChange } = props;
  return (
    <>
      <div className="field-row field-row-target">
        <label className="field">
          <span>SSH user</span>
          <input
            value={values.user}
        ...
        <label className="field">
          ...
          <input
            ...
            onChange={(event) => onChange({ ...values, host: event.target.value })}
          />
        </label>
        <label className="field">
          <span>Port</span>
          <input
            ...
            value={values.port}
            onChange={(event) =>
              onChange({ ...values, port: Number(event.target.value) || values.port })
            }
          />
        </label>
      </div>
      <label className="field">
        ...
        <div className="field-with-action">
          <input
            ...
            onChange={(event) => onChange({ ...values, keyPath: event.target.value })}
          />
          <Button
            onClick={async () => {
              const picked = await PickSSHKey().catch(() => '');
              if (picked) onChange({ ...values, keyPath: picked });
            }}
          >
            Browse…
          </Button>
        </div>
        <small>Leave empty to use your SSH agent and default keys.</small>
      </label>
    </>
  );
}

// TransferProgress is the live view of a running push/import job: the stage
// list plus a short tail of the actual output, and a promise that closing the
// sheet abandons nothing.
function TransferProgress(props: {
  job: Job;
  stages: readonly { id: string; title: string; description: string }[];
  stageIndex: (message: string, state: string) => number;
}) {
  const { job, stages } = props;
  const current = props.stageIndex(job.message, job.state);
  const tail = (job.log ?? []).slice(-6);
  return (
    <>
      <ol className="stages">
        {stages.map((stage, index) => {
          const stageState = stageStateAt(index, current, job.state);
          return (
            <li className={`stage stage-${stageState}`} key={stage.id}>
              <div className="stage-mark" aria-hidden>
                {stageState === 'done' ? (
              ...
              <div className="stage-text">
                <strong>{stage.title}</strong>
                <small>{stageState === 'active' ? job.message : stage.description}</small>
              </div>
            </li>
          );
        })}
      </ol>
      {job.state === 'running' && tail.length > 0 ? (
      ...
      {job.state === 'running' ? (
        <p className="muted">
          You can close this window — the job keeps running and lives under Activity.
        </p>
      ) : null}
      {job.state === 'error' ? (
        <div className="banner banner-warn">
          <div className="banner-body">
            <strong>The job failed</strong>
            <p>{job.message}</p>
          </div>
        </div>
      ) : null}
...
// PlanSummary shows what a migration will do — the informed-consent half of
// the push flow, and a nice-to-know for imports.
function PlanSummary({ plan }: { plan: TransferPlan }) {
  ...
  return (
    <div className="plan">
      <div className="plan-title">
        Plan: <strong>{plan.source}</strong> → <strong>{plan.dest}</strong>
        {plan.estimatedMB > 0 ? <span className="plan-size">~{formatMB(plan.estimatedMB)}</span> : null}
      </div>
      <ul className="plan-list">
        {plan.packages?.length ? (
          <li>
            <strong>{plan.packages.length} package{plan.packages.length === 1 ? '' : 's'}</strong>{' '}
            {plan.packages.join(', ')}
          </li>
        ) : null}
        {plan.users?.length ? (
          <li>
            <strong>Users</strong> {plan.users.join(', ')}
          </li>
        ) : null}
        {paths.length ? (
          <li>
            <strong>Files</strong> {paths.map((p) => p.path).join(', ')}
          </li>
        ) : null}
        {volumes.length ? (
          <li>
            <strong>Docker volumes</strong>{' '}
            {volumes.map((v) => `${v.name} (~${formatMB(v.sizeMB)})`).join(', ')}
          </li>
        ) : null}
        ...
        {compose.length ? (
          <li>
            ...
            {compose
              .map((c) => `${c.name}${c.running ? ' (will be started)' : ' (stays stopped)'}`)
              .join(', ')}
          </li>
        ) : null}
      </ul>
...
function formatMB(mb: number): string {
  ...
  return `${Math.max(mb, 1)} MB`;
}

const PUSH_TARGET_KEY = 'vpsbox.pushTarget';

function PushSheet(props: { instance: Sandbox; jobs: Job[]; onClose: () => void }) {
  const [values, setValues] = useState<VPSTargetValues>(() => loadStoredTarget(PUSH_TARGET_KEY));
  ...
  const job = props.jobs.find((candidate) => candidate.id === jobId);
  const ready = values.user.trim() !== '' && values.host.trim() !== '';
  ...
  const preview = async () => {
    ...
    setPlan(null);
    try {
      const result = await PreviewPush(targetInput());
      setPlan(result as unknown as TransferPlan);
    } catch (err) {
      setPreviewError(toMessage(err));
    } finally {
      setPreviewing(false);
    }
  };
  ...
  const push = async () => {
    ...
    setPreviewError('');
    try {
      saveStoredTarget(PUSH_TARGET_KEY, values);
      setJobId(await StartPushToVPS(targetInput()));
    } catch (err) {
      setPreviewError(toMessage(err));
    } finally {
      setStarting(false);
  ...
  return (
    <Sheet
      title={`Push ${props.instance.name} to a real VPS`}
      subtitle="Packages, files, docker volumes, and services move — SSH access and network config never do"
      onClose={props.onClose}
      wide
      footer={
        job?.state === 'done' ? (
          ...
          <>
            ...
            <Button variant="primary" onClick={() => setJobId('')}>
              Back to plan
            </Button>
          </>
        ) : job ? (
          <Button onClick={props.onClose}>Hide — keeps running</Button>
        ) : (
          <>
            <Button onClick={props.onClose}>Cancel</Button>
            {plan == null ? (
              <Button variant="primary" busy={previewing} disabled={!ready} onClick={() => void preview()}>
                Preview plan
              </Button>
            ) : (
              <Button variant="primary" busy={starting} onClick={() => void push()}>
                Push to {host}
              </Button>
            )}
          </>
            ...
            {job.state === 'done' ? (
              <div className="banner banner-ok">
                <div className="banner-body">
                  ...
                  <p>
                    {values.user.trim()}@{host} now carries what {props.instance.name} had.
                  </p>
                </div>
              </div>
            ) : null}
            <TransferProgress job={job} stages={PUSH_STAGES} stageIndex={pushStageIndex} />
          </>
        ) : (
          <>
            <VPSTargetFields
              ...
              onChange={(next) => {
                setValues(next);
                // A different target means the plan no longer describes reality.
                setPlan(null);
              }}
            />
            ...
            {previewError ? (
              <div className="banner banner-warn">
                <div className="banner-body">
                  <strong>Could not inspect the server</strong>
                  <p>{previewError}</p>
                  {previewError.toLowerCase().includes('ssh') ? (
                    <p className="muted">
                      Check that the host is reachable and the key matches — the connection is
                      non-interactive, so a key that needs a passphrase prompt cannot be used.
                    </p>
                  ) : null}
                </div>
              </div>
            ...
            {previewing ? (
              <div className="plan-loading">
                <span className="spin" aria-hidden /> Inspecting both machines and computing the
                plan…
              </div>
            ) : null}
            ...
            {plan ? (
              <>
                ...
                <p className="muted">
                  Pushing installs packages and overwrites files on {host}. It cannot remove your
                  access: SSH settings and keys on the server are never touched.
                </p>
              </>
            ) : null}
          </>
...
const IMPORT_TARGET_KEY = 'vpsbox.importTarget';
...
function ImportSheet(props: {
  ...
  const [values, setValues] = useState<VPSTargetValues>(() => loadStoredTarget(IMPORT_TARGET_KEY));
  ...
  const check = async () => {
    ...
    setPreview(null);
    try {
      const result = await PreviewImportVPS(targetInput());
  ...
  const start = async () => {
    ...
    setPreviewError('');
    try {
      saveStoredTarget(IMPORT_TARGET_KEY, values);
      setJobId(await StartImportFromVPS(targetInput()));
    } catch (err) {
      setPreviewError(toMessage(err));
    } finally {
      setStarting(false);
  ...
  return (
    <Sheet
      title="Import a real VPS"
      subtitle="Clone a server into a local sandbox you can break safely — the server is only read from"
      onClose={props.onClose}
      wide
      footer={
        job?.state === 'done' ? (
          <Button variant="primary" onClick={() => props.onImported(job.target)}>
            Open {job.target}
          </Button>
        ) : job?.state === 'error' ? (
          <>
          ...
          <>
            <Button onClick={props.onClose}>Cancel</Button>
            {preview == null ? (
              <Button variant="primary" busy={previewing} disabled={!ready} onClick={() => void check()}>
                Check server
              </Button>
            ) : (
              <Button variant="primary" busy={starting} onClick={() => void start()}>
                Import as {preview.sandboxName}
              </Button>
            )}
          </>
        )
            ...
            {job.state === 'done' ? (
              <div className="banner banner-ok">
                <div className="banner-body">
                  ...
                  <p>
                    {job.target} is a local copy of {values.user.trim()}@{values.host.trim()}.
                    Break it as much as you like — the real server never noticed you were there.
                  </p>
                </div>
              </div>
            ) : null}
            <TransferProgress job={job} stages={IMPORT_STAGES} stageIndex={importStageIndex} />
          </>
        ) : (
          <>
            <VPSTargetFields
            ...
            <label className="field">
              ...
              />
              <small>Leave empty to auto-name: vps-1, vps-2, …</small>
            </label>
            ...
            {previewing ? (
              <div className="plan-loading">
                <span className="spin" aria-hidden /> Reading the server — nothing on it changes…
              </div>
            ) : null}
            ...
            {preview ? (
              <div className="plan">
                <div className="plan-title">
                  Found <strong>{preview.os || 'a server'}</strong> — it becomes sandbox{' '}
                  <strong>{preview.sandboxName}</strong>
                  {preview.estimatedMB > 0 ? (
                    <span className="plan-size">~{formatMB(preview.estimatedMB)} to copy</span>
                  ) : null}
                </div>
                <ul className="plan-list">
                  <li>
                    <strong>Sandbox size</strong> {preview.cpus} vCPU, {preview.memoryGB} GB
                    memory, {preview.diskGB} GB disk — sized from the server
                  </li>
                  <li>
                    <strong>Packages</strong> {preview.packages} installed on the server
                  </li>
                  {preview.volumes?.length ? (
                    <li>
                      <strong>Docker volumes</strong>{' '}
                      {preview.volumes
                        .map((volume) => `${volume.name} (~${formatMB(volume.sizeMB)})`)
                        .join(', ')}
                    </li>
                  ) : null}
                  {preview.compose?.length ? (
                    <li>
                      <strong>Compose projects</strong>{' '}
                      {preview.compose
                        .map(
                          (project) =>
                            `${project.name}${project.running ? ' (will be started)' : ' (stays stopped)'}`,
                        )
                        .join(', ')}
                    </li>
                  ) : null}
              ...
              <p className="muted">
                VPSBox inspects the server, creates a sandbox sized to hold what runs there, then
                copies packages, files, docker volumes, and services in. Rehearse upgrades and
                break things locally — the real server never notices.
              </p>
            )}
          </>
        )}
